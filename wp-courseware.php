 ...
 *
 * Copyright 2012-2017 Fly Plugins - Lighthouse Media, LLC
 */

/** The current version of the database. */
define('WPCW_PLUGIN_VERSION', 			'4.0.6');		// Used for plugin updates
define('WPCW_DATABASE_VERSION', 		'4.000');	// Used for DB and scripts
...
// PDF
include_once 'pdf/pdf_create_certificate.php';
include_once 'pdf/pdf_create_results.php';
...
// Roles
require_once 'classes/class_wpcw_roles.php';
...
function WPCW_licence_edd_activateLicence()
{
	...
	$response = wp_remote_post( EDD_SL_STORE_URL_WPCW, array( 'timeout' => 15, 'sslverify' => false, 'body' => $api_params ) );
...
function WPCW_licence_edd_checkLicence()
{
	...
	{
		$license_status = __('You have exceeded your license limit. Please login to the member portal to <a href="http://flyplugins.com/member-portal">upgrade your license</a> or deactivate one of your current licenses.', 'wp-courseware');
	}
	else
	{
		...
		{
		  case 'invalid':
		  	$license_status = __('<a href="admin.php?page=WPCW_showPage_Settings">Register</a> your copy of WP Courseware by entering your licence key and activating it. Need a licence? <a href="http://wpcourseware.com">Purchase one now.</a>', 'wp-courseware');
		    break;

		  case 'expired':
		    $license_status = __('Your licence key has expired. Please <a href="http://wpcourseware.com">renew your licence.</a>', 'wp-courseware');
		    break;

		  case 'disabled':
		    $license_status = __('Your licence key is disabled.', 'wp-courseware');
		    break;

		  case 'site_inactive';
		  	$license_status = __('Your licence has not been activated on this site.', 'wp-courseware');
		  	break;
...
function WPCW_plugin_init()
{
	...
	/**
	 * @since 4.0.3 Moved localization below to it's own function
	 */
	 WPCW_load_textdomain();
	...
	*/
	...
	if (is_admin())
	{
		...
		add_filter( 'pre_get_posts',                            'WPCW_course_unit_permissions_filter_query' );
		add_filter( 'current_screen',                           'WPCW_course_unit_permissions_change_counts' );
		...
		//add_action('pre_user_query',                            'WPCW_users_permissionUsersFilter',20,1);
...
/**
 * @since 4.0.3
 * our textdomain changed from "wp_courseware" to "wp-courseware".
 *
 ...
 *
 * Here is the order in which the language files are referenced.
 * - wp-content/languages/wp_courseware/ (custom folder we have supported since initial release)
 * - wp-content/languages/plugins/wp-courseware
 * - wp-content/plugins/wp-courseware/languages/
 *
 * In wp-content/languages/wp_courseware/ we must look for "wp-courseware-{lang}_{country}.mo"
 * In wp-content/languages/wp_courseware/ we must look for "wp_courseware-{lang}_{country}.mo" as that was the old file naming convention
 * In wp-content/languages/plugins/wp-courseware/ we only need to look for "wp-courseware-{lang}_{country}.mo" as that is the new structure
 * In wp-content/plugins/wp-courseware/languages/, we must look for both naming conventions. This is done by filtering "load_textdomain_mofile"
 *
 */
function WPCW_load_textdomain()
{

		add_filter( 'load_textdomain_mofile', 'WPCW_load_old_textdomain' , 10, 2 );

		// Set filter for plugin's languages directory.
		$wpcw_lang_dir  = dirname( plugin_basename( __FILE__ ) ) . '/language/';
		$wpcw_lang_dir  = apply_filters( 'wpcw_languages_directory', $wpcw_lang_dir );
		...
		// Look in wp-content/languages/plugins/wp-courseware
		...
		if ( file_exists( $mofile_global1 ) ) {
			...
			load_plugin_textdomain( 'wp-courseware', false, $wpcw_lang_dir );
		}
}
 ...
 * https://github.com/10up/grunt-wp-plugin/issues/21#issuecomment-62003284
 */
...
function WPCW_menu_MainMenu()
{
	...
	// Menu Icon
	$wpcw_menu_icon = WPCW_plugin_getPluginPath() . 'img/icon_training_16.png';

	// Since V2.8 - Allow access level to be changed via filter for pages.
	$fa_training_courses  = apply_filters( 'wpcw_back_menus_access_training_courses', 'view_wpcw_courses' );
	$fa_gradebook         = apply_filters( 'wpcw_back_menus_access_gradebook', 'view_wpcw_courses' );
	$fa_user_progress     = apply_filters( 'wpcw_back_menus_access_user_progress', 'view_wpcw_courses' );
	$fa_user_quiz_results = apply_filters( 'wpcw_back_menus_access_user_quiz_results', 'view_wpcw_courses' );

	// ### Main Menu Page
	$WPCW_course_dashboard_page = add_menu_page(
		__( 'WP Courseware', 'wp-courseware' ),
		__( 'Training Courses', 'wp-courseware' ),
		$fa_training_courses,
		WPCW_PLUGIN_ID,
		'WPCW_showPage_Dashboard',
		$wpcw_menu_icon,
		WPCW_MENU_POSITION
	);
	...
	add_submenu_page(
		WPCW_PLUGIN_ID,
		__( 'WP Courseware - New Course', 'wp-courseware' ),
		__( 'Add Course', 'wp-courseware' ),
		'view_wpcw_courses',
		'WPCW_showPage_ModifyCourse',
		'WPCW_showPage_ModifyCourse'
	);
	...
	add_submenu_page(
		WPCW_PLUGIN_ID,
		__( 'WP Courseware - Gradebook', 'wp-courseware' ),
		__( 'Gradebook', 'wp-courseware' ),
		$fa_gradebook,
		'WPCW_showPage_GradeBook',
		'WPCW_showPage_GradeBook'
	);
	...
	add_submenu_page(
		WPCW_PLUGIN_ID,
		__( 'WP Courseware - Modify Module', 'wp-courseware' ),
		__( 'Add Module', 'wp-courseware' ),
		'view_wpcw_courses',
	...
	add_submenu_page(
		WPCW_PLUGIN_ID,
		false,
		'<span class="wpcw_menu_section" style="display: block; margin: 1px 0 1px -5px; padding: 0; height: 1px; line-height: 1px; background: #CCC;"></span>',
		'view_wpcw_courses',
		'#',
		false
	);
	...
	add_submenu_page(
		WPCW_PLUGIN_ID,
		__( 'WP Courseware - Modify Quiz', 'wp-courseware' ),
		__( 'Add Quiz/Survey', 'wp-courseware' ),
		'view_wpcw_courses',
		'WPCW_showPage_ModifyQuiz',
		'WPCW_showPage_ModifyQuiz'
	);
	...
	add_submenu_page(
		WPCW_PLUGIN_ID,
		__( 'WP Courseware - Quiz Summary', 'wp-courseware' ),
		__( 'Quiz Summary', 'wp-courseware' ),
		'view_wpcw_courses',
		'WPCW_showPage_QuizSummary',
		'WPCW_showPage_QuizSummary'
	);
	...
	add_submenu_page(
		WPCW_PLUGIN_ID,
		__( 'WP Courseware - Question Pool', 'wp-courseware' ),
		__( 'Question Pool', 'wp-courseware' ),
		'view_wpcw_courses',
	...
	add_submenu_page(
		WPCW_PLUGIN_ID,
		__( 'WP Courseware - Add Question', 'wp-courseware' ),
		__( 'Add Question', 'wp-courseware' ),
		'view_wpcw_courses',
		'WPCW_showPage_ModifyQuestion',
		'WPCW_showPage_ModifyQuestion'
	);
	...
	add_submenu_page(
		WPCW_PLUGIN_ID,
		__( 'WP Courseware - Module &amp; Unit Ordering', 'wp-courseware' ),
		__( 'Module &amp; Unit Ordering', 'wp-courseware' ),
		'view_wpcw_courses', 'WPCW_showPage_CourseOrdering',
		'WPCW_showPage_CourseOrdering'
	);
	...
	if ( count( $extensionList ) > 0 ) {
		foreach ( $extensionList as $extensionItem ) {
			add_submenu_page(
				WPCW_PLUGIN_ID,
				__( 'WP Courseware - ', 'wp-courseware' ) . $extensionItem[ 'page_title' ],
				$extensionItem[ 'menu_label' ],
				'view_wpcw_courses',
				$extensionItem[ 'id' ],
				$extensionItem[ 'menu_function' ]
			);
		}
	...
	add_submenu_page(
		WPCW_PLUGIN_ID,
		__( 'WP Courseware - Import/Export Course', 'wp-courseware' ),
		__( 'Import/Export', 'wp-courseware' ),
		'view_wpcw_courses',
		'WPCW_showPage_ImportExport',
		'WPCW_showPage_ImportExport'
	);
	...
	add_submenu_page(
		WPCW_PLUGIN_ID,
		__( 'WP Courseware - Convert Page/Post to Course Unit', 'wp-courseware' ),
		__( 'Convert Page/Post', 'wp-courseware' ),
		'view_wpcw_courses',
		'WPCW_showPage_ConvertPage',
		'WPCW_showPage_ConvertPage'
	);
	...
	add_submenu_page(
		WPCW_PLUGIN_ID,
		__( 'WP Courseware - Settings', 'wp-courseware' ),
		__( 'Settings', 'wp-courseware' ),
		'manage_wpcw_settings',
		'WPCW_showPage_Settings',
		'WPCW_showPage_Settings'
	);

	// ### Certificate Settings - Menu removed in 4.0
	//add_submenu_page(
	//	WPCW_PLUGIN_ID,
	//	__( 'WP Courseware - Certificates', 'wp-courseware' ),
	//	__( 'Certificates', 'wp-courseware' ),
	//	'manage_wpcw_settings',
	//	'WPCW_showPage_Certificates',
	//	'WPCW_showPage_Certificates'
	//);
	...
	add_submenu_page(
		WPCW_PLUGIN_ID,
		__( 'WP Courseware - Documentation', 'wp-courseware' ),
		__( 'Documentation', 'wp-courseware' ),
		'view_wpcw_courses',
	...
	add_users_page(
		__( 'WP Courseware - Update User Course Access Permissions', 'wp-courseware' ),
		__( 'Update Course Access', 'wp-courseware' ),
		'view_wpcw_courses',
		'WPCW_showPage_UserCourseAccess',
		'WPCW_showPage_UserCourseAccess'
	);

	// ### Detailed user progress
	add_users_page(
		__( 'WP Courseware - View User Progress', 'wp-courseware' ),
		__( 'View User Progress', 'wp-courseware' ),
		$fa_user_progress,
		'WPCW_showPage_UserProgess',
		'WPCW_showPage_UserProgess'
	);

	// ### Detailed quiz progress
	add_users_page(
		__( 'WP Courseware - View Quiz/Survey Results', 'wp-courseware' ),
		__( 'View Quiz Results', 'wp-courseware' ),
		$fa_user_quiz_results,
...
function WPCW_menu_MainMenu_NetworkOnly()
{
	...
	add_menu_page(
		...
		'manage_options',
		WPCW_PLUGIN_ID,
		'WPCW_showPage_Settings_Network',
		$wpcw_menu_icon, WPCW_MENU_POSITION
	);
}
...
function WPCW_addCustomScripts_BackEnd()
{
	...
	switch ($screen->id)
	{
		...
		case 'users':
				...
				array(
						'confirm_bulk_change' 			=> __('Are you sure you wish to reset the progress of the selected users? This CANNOT be undone.', 'wp-courseware'),
						'confirm_single_change' 		=> __('Are you sure you wish to reset the progress of this user? This CANNOT be undone.', 'wp-courseware'),
						'status_copying' 				=> __('Copying...', 'wp-courseware'),
				));
			break;
	...
	if ($needDatePicker)
	{
		...
		$aryArgs = array(
	        'closeText'         => __('Done', 						'wp-courseware'),
	        'currentText'       => __('Today', 						'wp-courseware'),
			'monthStatus'       => __( 'Show a different month', 	'wp-courseware'),

			'timeText'			=> __('Time', 						'wp-courseware'),
			'hourText'			=> __('Hour', 						'wp-courseware'),
			'minuteText'		=> __('Minute', 					'wp-courseware'),
			'secondText'		=> __('Second', 					'wp-courseware'),
	...
	array(
			...
			'confirm_whole_course_reset' 	=> __('Are you sure you wish to reset the progress of all users on this course? This CANNOT be undone.', 'wp-courseware'),
			'confirm_access_change_users' 	=> __('Are you sure you wish to add access for this course for all users?', 'wp-courseware'),
			'confirm_access_change_admins' 	=> __('Are you sure you wish to add access for this course for all admins?', 'wp-courseware'),
			'msg_question_duplicate' 		=> __('That question already exists in this quiz, so cannot be added again.', 'wp-courseware'),
			'name_tag_whole_pool' 			=> __('Entire Question Pool', 'wp-courseware'),
	));
}
...
function WPCW_addCustomScripts_FrontEnd()
{
	...
	array(
			...
			'str_uploading'			=> __('Uploading:', 'wp-courseware'),		// Uploading message.
			'str_quiz_all_fields'	=> __('Please provide an answer for all of the questions on this page.', 'wp-courseware'),

			// Timer units
			'timer_units_hrs' 		=> __('hrs', 'wp-courseware'),
			'timer_units_mins' 		=> __('mins', 'wp-courseware'),
			'timer_units_secs' 		=> __('secs', 'wp-courseware'),
...
{
	$labels = array(
        'name' 					=> __( 'Course Units', 						'wp-courseware'),
        'singular_name' 		=> __( 'Course Unit', 						'wp-courseware'),
        'add_new' 				=> __( 'Add New', 							'wp-courseware'),
        'add_new_item' 			=> __( 'Add New Course Unit', 				'wp-courseware'),
        'edit_item'	 			=> __( 'Edit Course Unit', 					'wp-courseware'),
        'new_item' 				=> __( 'New Course Unit', 					'wp-courseware'),
        'view_item' 			=> __( 'View Course Unit', 					'wp-courseware'),
        'search_items' 			=> __( 'Search Course Units', 				'wp-courseware'),
        'not_found' 			=> __( 'No course units found', 			'wp-courseware'),
        'not_found_in_trash'	=> __( 'No course units found in Trash', 	'wp-courseware'),
        'parent_item_colon' 	=> __( 'Parent Course Unit:', 				'wp-courseware'),
        'menu_name' 			=> __( 'Course Units', 						'wp-courseware'),
	);
...
function WPCW_menu_MainMenu_cleanUnwantedEntries()
{
	...
	WPCW_menu_removeSubmenuItem('profile.php', 'WPCW_showPage_UserCourseAccess');
...
function WPCW_plugin_rolesCheck() {
	...
	// Check for the roles object, if it doesn't exist somehow, exit.
	if ( ! is_object( $wp_roles ) ) {
		return;
	}

	// Create Roles & Capabilities, if the instructor role does not exist.
...
add_action( 'admin_init', 'WPCW_plugin_rolesCheck' );
...
function WPCW_plugin_uninstallChecks() {
	// Remove Roles
	WPCW_Roles()->remove_caps();
}
register_uninstall_hook(__FILE__, 'WPCW_plugin_uninstallChecks');
