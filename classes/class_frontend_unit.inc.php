...
class WPCW_UnitFrontend
{
    ...
    public function render_detailsForUnit($content, $justGetCompletionAndQuizData = false)
    {
        ...
        if ($this->check_unit_isCompleted()) {
            ...
            if ($lockDetails['content_locked']) {
                ...
                $lockedMsg = self::message_createMessage_error($this->parentData->course_message_unit_not_yet_dripfeed);
                ...
                $content = false;
            } // User can access content already, so don't worry
            else {
    ...
    public function render_quizzes_showAllCorrectAnswers($returnDataInArray = false)
    {
        ...
        if (!$returnDataInArray) {
            ...
            $html .= sprintf('<div class="wpcw_fe_quiz_title"><b>%s</b> %s</div>', __('Correct Answers for: ', 'wp-courseware'), $this->unitQuizDetails->quiz_title);
        ...
        if ($this->unitQuizDetails->questions && count($this->unitQuizDetails->questions) > 0) {
            ...
            foreach ($this->unitQuizDetails->questions as $question) {
                ...
                // ### 3a - Question title
                $html .= sprintf(
                        '<div class="wpcw_fe_quiz_q_title">%s #%d: %s</div>', __('Question', 'wp-courseware'), $questionNum++, nl2br(htmlspecialchars($question->question_question))
                );
                ...
                if ($setting_showExplanation && $question->question_answer_explanation) {
                    $html .= sprintf(
                            '<div class="wpcw_fe_quiz_q_explanation"><b>%s:</b>&nbsp;&nbsp;%s</div>', __('Explanation', 'wp-courseware'), nl2br($question->question_answer_explanation)
                    );
                }

                // Add this portion of data rather than use the div wrapper.
                if ($returnDataInArray) {
                    $arrayToReturn[] = apply_filters( 'the_content', $html );
                } else {
    ...
    public function render_completionBox_complete()
    {
        ...
        if ($this->userProgress->isCourseCompleted()) {
            ...
            if ('use_certs' == $this->parentData->course_opt_use_certificate && $certificateDetails) {
                $certHTML = sprintf(
                        '<div class="wpcw_fe_progress_box_download"><a href="%s" class="fe_btn fe_btn_download" target="_blank">%s</a></div>', WPCW_certificate_generateLink($certificateDetails->cert_access_key), __('Download Certificate', 'wp-courseware')
                );
            }
            ...
            $html = sprintf(
                    '<div class="wpcw_fe_progress_box_wrap"><div class="wpcw_fe_progress_box wpcw_fe_progress_box_complete">%s</div></div>', $this->parentData->course_message_unit_complete
            );
        }
        ...
        if ($this->unitQuizProgress && 'retake_waiting' != $this->unitQuizProgress->quiz_next_step_type && 'incomplete' != $this->unitQuizProgress->quiz_paging_status) {
        ...
        if ($this->check_quizzes_NonBlockingQuizOffersRetakeButton()) {
                ...
                if ('quiz_noblock' == $this->unitQuizDetails->quiz_type && 'incomplete' != $this->unitQuizProgress->quiz_paging_status) {
                    ...
                    if ($this->unitQuizProgress->quiz_needs_marking != 0) {
                        ...
                        $messageToShow .= wpautop(sprintf(
                                        __('The recommended grade for this quiz is <b>%d%%</b> (and your grade is <b>%d%%</b>). The course instructor has allowed you to retake this quiz if you wish to improve your grade.', 'wp-courseware'), $this->unitQuizDetails->show_recommended_percentage, $this->unitQuizProgress->quiz_grade
                        ));
            ...
            if ($showRetakeButton) {
				...
				</div>', WPCW_plugin_getPluginPath(), $this->unitQuizDetails->quiz_id, $this->unitPost->ID, __('Retake Quiz', 'wp-courseware')
                );
    ...
    public function check_quizzes_workoutQuizPassStatusDetails()
    {
        ...
        if ($this->check_quizzes_validQuizDetails() &&
            ...
            if ($this->unitQuizProgress->quiz_needs_marking == 0) {
                ...
                $rtnDetails['button_dl_results_url'] = apply_filters('wpcw_results_generated_url', add_query_arg( array( 'page' => 'wpcw_pdf_create_results', 'unitid' => $this->unitPost->ID, 'quizid' => $this->unitQuizDetails->quiz_id ), esc_url( site_url( '/' ) ) ) );
                ...
                if ($this->unitQuizDetails->has_open_questions) {
                    $rtnDetails[0]['msg_overall_grade'] = sprintf(
                            __('Your grade for this quiz is <b>%d%%</b>.', 'wp-courseware'), $this->unitQuizProgress->quiz_grade
                    );
                } // Just closed questions, show out of message.
                else {
                    $rtnDetails[0]['msg_overall_grade'] = sprintf(
                            __('You got %1$d out of %2$d questions <b>(%3$d%%)</b> correct!', 'wp-courseware'), $this->unitQuizProgress->quiz_correct_questions, $this->unitQuizProgress->quiz_question_total, $this->unitQuizProgress->quiz_grade
                    );
                }
                ...
                if ('quiz_noblock' == $this->unitQuizDetails->quiz_type) {
                    ...
                    if ($this->check_quizzes_NonBlockingQuizOffersRetakeButton() && $attemptCount > 0) {
                        $rtnDetails[1]['msg_attempts'] = ($attemptCount == 1 ? __('You have had <b>1 previous attempt</b> at this quiz.', 'wp-courseware') : sprintf(__('You have had <b>%d previous attempts</b> at this quiz.', 'wp-courseware'), $attemptCount)
                                );
                    }
                } // 4) Blocking quiz - show if they have passed or failed this quiz.
                else {
                    // 5) Show the pass mark.
                    $rtnDetails[0]['msg_block_pass_grade'] = sprintf(__('The pass grade for this quiz is <b>(%d%%)</b>', 'wp-courseware'), $this->unitQuizDetails->quiz_pass_mark);

                    // 6) Show if they have passed or failed.
                    if ($this->check_quizzes_hasUserPassedQuiz()) {
                        $rtnDetails[1]['msg_block_pass_has_passed'] = sprintf(__('Congratulations, you have <b>passed</b> this quiz!', 'wp-courseware'), $this->unitQuizDetails->quiz_pass_mark);
                        $rtnDetails[1]['msg_block_pass_has_passed_unit_complete'] = $this->parentData->course_message_unit_complete;
                    } else {
                        $errorMode = true;
                        $rtnDetails[1]['msg_block_pass_has_failed'] = sprintf(__('Unfortunately, this means you have <b>failed</b> this quiz.', 'wp-courseware'), $this->unitQuizDetails->quiz_pass_mark);
                        ...
                        if ('progress_anyway' == $this->unitQuizProgress->quiz_next_step_type) {
                            ...
                            $rtnDetails[1]['msg_block_pass_next_action'] = sprintf(__('To progress to the next unit, you need to pass this quiz.', 'wp-courseware'), $this->unitQuizDetails->quiz_pass_mark);
                        }

                        // 7) How many attempts have they now had?
                        $rtnDetails[2]['msg_attempts'] = ($attemptCount == 1 ? __('You have had <b>1 previous attempt</b> at this quiz.', 'wp-courseware') : sprintf(__('You have had <b>%d previous attempts</b> at this quiz.', 'wp-courseware'), $attemptCount)
                                );
                    }
                ...
                if ('quiz_block' == $this->unitQuizDetails->quiz_type && // Timers only appear on blocking quizzes
                    ...
                    $rtnDetails['msg_results_by_timer'] = sprintf(
                            '<b>%s:</b> %s (%s %s)', __('Completion Time', 'wp-courseware'), WPCW_time_convertSecondsToHumanLabel($this->unitQuizProgress->quiz_completion_time_seconds), WPCW_time_convertMinutesToHumanLabel($this->unitQuizDetails->quiz_timer_mode_limit), __('allowed', 'wp-courseware')
                    );
                } // end of timer check
    ...
    public function render_completionBox_quizPassStatus()
    {
        ...
        if (isset($msgDetails['button_dl_results_url']) && $switch == 'on')
        {
            $messageToShow .= sprintf(
				...
				</div>', $msgDetails['button_dl_results_url'], __('Download Results', 'wp-courseware')
            );
        }
        ...
        if ($this->check_quizzes_validQuizDetails()) {
            if ($this->check_quizzes_hasUserCompletedQuiz()) {
                ...
                if ($this->unitQuizProgress->quiz_needs_marking > 0) {
                    ...
                    if ('quiz_block' == $this->unitQuizDetails->quiz_type) {
                        $html .= self::message_createMessage_success($this->parentData->course_message_quiz_open_grading_blocking);
                    }
                }
            }
            ...
            $html .= sprintf(
				...
				<div class="wpcw_fe_progress_box_wrap" id="wpcw_fe_unit_complete_%d">
					...
					%s</div>
				</div>', $this->unitPost->ID, WPCW_plugin_getPluginPath().'img', $this->unitPost->ID, __('Mark as Completed', 'wp-courseware'), $this->parentData->course_message_unit_pending
            );
        }
    ...
    public function render_quizzes_handleQuizRendering($showErrorsOnForm = true)
    {
        ...
        if ('quiz_block' == $this->unitQuizDetails->quiz_type) {
            ...
            $html .= sprintf(
                    __('You\'ll need to correctly answer at least <b>%1$d of the %2$d</b> questions below (<b>%4$s%3$d%%</b>) to progress to the next unit.', 'wp-courseware'), $passQs, $totalQs, $this->unitQuizDetails->quiz_pass_mark, $atLeast
            );
            $html .= '</div>';
        }
                ...
                if (empty($this->unitQuizProgress) || 'retake_waiting' == $this->unitQuizProgress->quiz_next_step_type) {
                    ...
                    $html .= sprintf(
                            '
									...
									</div>', sprintf(__('You have <b>%s</b> to complete this quiz...', 'wp-courseware'), WPCW_time_convertMinutesToHumanLabel($this->unitQuizDetails->quiz_timer_mode_limit)), $this->unitQuizDetails->quiz_id, $this->unitPost->ID, __('Begin Quiz...', 'wp-courseware'), WPCW_plugin_getPluginPath()
                    );
                ...
                if ($this->check_paging_areWePagingQuestions()) {
                    ...
                    $html .= sprintf(
                            '<div class="wpcw_fe_quiz_submit wpcw_fe_quiz_submit_data">
									...
									</div>', WPCW_content_progressBar(0, 'wpcw_fe_upload_progress'), WPCW_plugin_getPluginPath(), __('Submit Answers', 'wp-courseware')
                    );
    ...
    public function render_navigation_getNavigationBox()
    {
        ...
        if ($nextAndPrev['prev'] > 0) {
            $html .= sprintf(
                    '<a href="%s" class="fe_btn fe_btn_navigation">&laquo; %s</a> ', get_permalink($nextAndPrev['prev']), __('Previous Unit', 'wp-courseware')
            );
        }
        ...
        if (!$disableNextButton && $nextAndPrev['next'] > 0) {
            ...
            $html .= sprintf(
                    '<a href="%s" class="fe_btn fe_btn_navigation %s">%s &raquo;</a>', get_permalink($nextAndPrev['next']), false, __('Next Unit', 'wp-courseware')
            );
        }
    ...
    {
        return self::message_createMessage_error(__('An error occured, so your unit progress was not updated. Please refresh the page and try again.', 'wp-courseware'));
    }
    ...
    public function message_user_notLoggedIn()
    {
        ...
        $newContent = self::message_createMessage_error($this->parentData->course_message_unit_not_logged_in);
    ...
    {
        // Return no access error without content
        $newContent = self::message_createMessage_error($this->parentData->course_message_unit_no_access);
    ...
    {
        // Return no access error without content
        $newContent = self::message_createMessage_error($this->parentData->course_message_unit_not_yet);
    ...
    {
        // Return no access error without content
        $newContent = self::message_createMessage_error($this->parentData->course_message_prerequisite_not_met);
     ...
     */
    public function check_quizzes_canWeContinue_checkAnswersFromPaging($potentialAnswers)
    {
        global $wpdb, $wpcwdb;
        $wpdb->show_errors();
        ...
        foreach ($this->unitQuizDetails->questions as $questionID => $question) {
            ...
            switch ($question->question_type) {
                ...
                case 'multi':
                     ...
                     */
                    if (count($checkedAnswerList[$questionID]) == count($correctAnswersArray)) {
                        /*
    ...
    function check_paging_find_next_unanswered_question(){
        ...
        if ($this->unitQuizProgress->quiz_paging_incomplete > 1){
                foreach ($this->unitQuizProgress_incompleteQs as $key => $question_id) {
    ...
    public function fetch_paging_getQuestion_moveQuestionMarker($jumpMode)
    {
        ...
        if (empty($this->unitQuizProgress) || 
                ('latest' == $this->unitQuizProgress->quiz_is_latest && 'retake_waiting' == $this->unitQuizProgress->quiz_next_step_type)) {
