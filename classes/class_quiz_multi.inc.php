...
class WPCW_quiz_MultipleChoice extends WPCW_quiz_base
{
    ...
    public function editForm_toString()
    {
        ...
        $html .= sprintf(
                '<input type="hidden" name="question_order_%s" value="%s" class="wpcw_question_hidden_order" />', $this->quizItem->question_id, $this->quizItem->question_order + 0
        );
        ...
        if ($answerList) {
            ...
            foreach ($answerList as $answerItem) {
                ...
                $html .= sprintf(
					...
					</td>', __('Add a new answer...', 'wp-courseware'), WPCW_plugin_getPluginPath(), __('Remove this answer...', 'wp-courseware'), WPCW_plugin_getPluginPath()
                );
    ...
    public function processAnswersWithRandomOption($unit_id,$quiz_id)
    {
        ...
        // Get all possible answers if we have them
        $possibleAnswers = $unitQuizProgress->quiz_data[$this->quizItem->question_id]['possible_answers'];
