...
class WPCW_quiz_base {
    ...
    public function __construct($quizItem) {
	    ...
	    if ( ! isset( $this->quizItem->question_author ) ) {
		    $this->quizItem->question_order = get_current_user_id();
	    }
    }
    ...
    public function getSection_actionButtons($columnCount) {
        ...
        // Check author of question, if not the author, remove the delete button.
	    $current_user = wp_get_current_user();
	    if ( ! user_can( $current_user, 'manage_wpcw_settings' ) && $current_user->ID != $this->quizItem->question_author ) {
		    return sprintf(
    ...
    protected function renderForm_toString_withClass($parentQuiz, $questionNum, $selectedAnswer, $showAsError, $cssClass, $errorToShow = false) {
        ...
        // Question title
        $html .= sprintf(
                '<div class="wpcw_fe_quiz_q_title">%s #%d: %s%s</div>', __('Question', 'wp-courseware'), $questionNum, nl2br(htmlspecialchars($this->quizItem->question_question)), ($wrongAnswerState ? '<span class="wpcw_fe_quiz_status">(' . __('Incorrect', 'wp-courseware') . ')</span>' : '')
        );
    ...
    protected function renderForm_toString_answerList($parentQuiz, $questionNum, $selectedAnswer, $showAsError, $cssClass) {
        ...
        if ($this->answerList) {
            ...
            foreach ($this->answerList as $answerItem => $answerValue) {
                ...
                $flagMultAns = false;
                if (is_array($ansArray) && count($ansArray) > 1) {
                    $flagMultAns = true;
                }
