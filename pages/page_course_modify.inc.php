...
function WPCW_showPage_ModifyCourse_load() {
	...
	if ( isset( $_GET[ 'course_id' ] ) ) {
		...
		if ( ! $courseDetails ) {
			$page->showPageHeader( __( 'Edit Course', 'wp-courseware' ), '75%', WPCW_icon_getPageIconURL() );
		...
		// Adding course
		$page->showPageHeader( __( 'Add Course', 'wp-courseware' ), '75%', WPCW_icon_getPageIconURL() );
	...
	if ( $courseDetails && $action = WPCW_arrays_getValue( $_GET, 'action' ) ) {
		...
		$formDetails = array(
			...
			'course_overview_page' => array(
					'label' 	=> __('Course Overview Page', 'wp-courseware'),
					'type'  	=> 'select',
					'required'  => false,
					'desc'  	=> __('The page that links to the list of all modules for the course.', 'wp-courseware'),
       ...
       if (!is_plugin_active('WPCourseWareNote/wp-courseware-note.php')) {
		...
		// Generate the tabs.
		$tabList = array(
			'break_course_general'                    => array( 'label' => __( 'General Course Details', 'wp-courseware' ) ),
		...
		$form->msg_record_created = __( 'Course details successfully created. ', 'wp-courseware' ) . $directionMsg;
		$form->msg_record_updated = __( 'Course details successfully updated. ', 'wp-courseware' ) . $directionMsg;
		...
		// Certificate Details
		$preview_certificate_url = add_query_arg( array( 'page' => 'wpcw_pdf_create_certificate', 'certificate' => 'preview', 'course_id' => $form->primaryKeyValue ), esc_url( site_url( '/' ) ) );
...
function WPCW_showPage_ModifyCourse_courseAccess_runAccessChanges( $page, $action, $courseDetails ) {
	$args = array(// No defaults actually, get_users() gets all users by default.
	);
