...
function WPCW_showPage_ModifyQuiz_load() {
	...
	if ( $quizDetails = WPCW_quizzes_getQuizDetails( $quizID, false, false, false ) ) {
		...
		if ( ! $quizDetails ) {
			$page->showPageHeader( __( 'Edit Quiz/Survey', 'wp-courseware' ), '70%', WPCW_icon_getPageIconURL() );
		...
		// Adding quiz
		$page->showPageHeader( __( 'Add Quiz/Survey', 'wp-courseware' ), '70%', WPCW_icon_getPageIconURL() );
	...
	$tabList = array(
		...
		'wpcw_section_break_quiz_custom_feedback' => array( 'label'    => __( 'Custom Feedback', 'wp-courseware' ),
		                                                    'cssclass' => 'wpcw_quiz_only_tab'
		),
			...
			'suffix_subitems' => array(
				...
				/*'no_answers' 	=> array(
					'no_answers_settings' => array(
						...
						'cssclass'		=> '',
							'data'	=> array(
								'no_show_explanation' 	=> '<b>' . __('Show explanation', 'wp-courseware') . '</b> - ' . __('Show the trainee an explanation for the correct answer (if there is one).', 'wp-courseware'),
							),
					),
				), // end of show_answers array*/
	...
	$directionMsg = '<br/></br>' . sprintf( __( 'Do you want to return to the <a href="%s">quiz summary page</a>?', 'wp-courseware' ), admin_url( 'admin.php?page=WPCW_showPage_QuizSummary' ) );

	// Override success messages
	$form->msg_record_created = __( 'Quiz details successfully created.', 'wp-courseware' ) . $directionMsg;
	$form->msg_record_updated = __( 'Quiz details successfully updated.', 'wp-courseware' ) . $directionMsg;
...
function WPCW_showPage_ModifyQuiz_showQuestionEntryForms($quizID, $page)
{
	...
	if ('quiz_block' == $quizDetails->quiz_type)
	{
		...
		printf(__('The trainee will be required to correctly answer at least <b>%d of the %d</b> following questions (<b>at least %d%%</b>) to progress.', 'wp-courseware'),
	...
	<div class="wpcw_floating_menu" id="wpcw_add_quiz_menu">
		...
		<div class="wpcw_add_quiz_block">
			...
			<div class="wpcw_add_quiz_options"><ul>
				...
				<li><a href="#TB_inline?width=1200&height=800&inlineId=wpcw_tb_question_pool" title="<?php _e('Insert question from Question Pool', 'wp-courseware'); ?>" class="button-secondary thickbox" id="wpcw_add_question_from_pool"><?php _e('Add Questions from Pool', 'wp-courseware'); ?></a></li>
				<li><a href="#TB_inline?width=1200&height=800&inlineId=wpcw_tb_random_question" title="<?php _e('Insert Random Questions', 'wp-courseware'); ?>" class="button-secondary thickbox" id="wpcw_add_question_random"><?php _e('Add Random Questions', 'wp-courseware'); ?></a></li>

				<li class="wpcw_add_quiz_spacer"><hr/></li>

				<li class="wpcw_quiz_tool_compact">
					<a href="#top" class="wpcw_quiz_tool_compact_compact" title="<?php _e('Use this to compact the questions so that they are easier to organise.', 'wp-courseware'); ?>">[-] <?php _e('Compact Questions', 'wp-courseware'); ?></a>
					<a href="#top" class="wpcw_quiz_tool_compact_expand" title="<?php _e('Use this to expand the questions for editing.', 'wp-courseware'); ?>">[+] <?php _e('Expand Questions', 'wp-courseware'); ?></a>
				</li>
			</ul></div>
		</div>
