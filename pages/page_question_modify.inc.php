...
function WPCW_showPage_ModifyQuestion_load() {
	...
	if ( isset( $_GET[ 'question_id' ] ) || isset( $_POST[ 'question_id' ] ) ) {
		...
		// Page Header
		$page->showPageHeader( __( 'Edit Single Question', 'wp-courseware' ), '70%', WPCW_icon_getPageIconURL() );
		...
		if ( 'true' == WPCW_arrays_getValue( $_POST, 'question_save_mode' ) ) {
			if ( 'true' == WPCW_arrays_getValue( $_POST, 'question_is_new_question' ) ) {
				...
				// Message
				$directionMsg = '<br/></br>' . sprintf( __( 'Do you want to return to the <a href="%s">question pool</a>?', 'wp-courseware' ), admin_url( 'admin.php?page=WPCW_showPage_QuestionPool' ) );

				// Output message
				$page->showMessage( __( 'Question successfully added.', 'wp-courseware' ) . $directionMsg );
			} else {
				// Save Data
				...
				// Output Mesage.
				$page->showMessage( __( 'Question successfully updated.', 'wp-courseware' ) . $directionMsg );
			}
		...
		// Page Header
		$page->showPageHeader( __( 'Add a Question', 'wp-courseware' ), '75%', WPCW_icon_getPageIconURL() );
	...
	printf( '<a href="%s?page=WPCW_showPage_QuestionPool" class="button-secondary">%s</a>&nbsp;&nbsp;', admin_url( 'admin.php' ), __( '&laquo; Return to Question Pool', 'wp-courseware' ) );
	if ( $newQuestion ) {
	...
	<div class="wpcw_floating_menu" id="wpcw_add_quiz_menu">
		<div class="wpcw_add_quiz_block">
			...
			<div class="wpcw_add_quiz_options">
				<ul>
					<li>
						<a href="<?php echo add_query_arg( array( 'question_type' => 'multi' ), $addQuestionUrl ); ?>" class="button-secondary" id="wpcw_add_question_multi"><?php _e( 'Add Multiple Choice', 'wp-courseware' ); ?></a>
					</li>
					<li>
						<a href="<?php echo add_query_arg( array( 'question_type' => 'truefalse' ), $addQuestionUrl ); ?>" class="button-secondary" id="wpcw_add_question_truefalse"><?php _e( 'Add True/False', 'wp-courseware' ); ?></a>
					</li>
					<li>
						<a href="<?php echo add_query_arg( array( 'question_type' => 'open' ), $addQuestionUrl ); ?>" class="button-secondary" id="wpcw_add_question_open"><?php _e( 'Add Open Ended Question', 'wp-courseware' ); ?></a>
					</li>
					<li>
						<a href="<?php echo add_query_arg( array( 'question_type' => 'upload' ), $addQuestionUrl ); ?>" class="button-secondary" id="wpcw_add_question_upload"><?php _e( 'Add File Upload Question', 'wp-courseware' ); ?></a>
					</li>
				</ul>
			</div>
		</div>
