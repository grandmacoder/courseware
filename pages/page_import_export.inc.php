...
function WPCW_showPage_ImportExport_importUsers()
{
	...
	printf('<br/><br/><div class="wpcw_docs_wrapper">');
		...
		printf('<ul>');
			...
			printf('<li>' . __('The IDs for the training courses can be found on the <a href="%s">course summary page</a>.', 'wp-courseware'). '</li>', admin_url('admin.php?page=WPCW_wp_courseware'));
		printf('</ul>');
	printf('</div>');
...
function WPCW_courses_importCourseFromFile($page)
{
	if (isset($_FILES['import_course_xml']['name']))
	{
		...
		if ($errornum == 0)
		{
			...
			{
				$message = __('The course was successfully imported.', 'wp-courseware') . '<br/><br/>';
				$message .= sprintf(__('You can now <a href="%s">edit the Course Settings</a> or <a href="%s">edit the Unit &amp; Module Ordering</a>.', 'wp-courseware'),
