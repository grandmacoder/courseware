...
$j(function()
{
	...
	$j('#wpcw_section_break_quiz_questions,#wpcw_quiz_details_questions').on('click', '.wpcw_question_add', function(e)
	{
		...
		newAnswer.find('.wpcw_quiz_details_tick_correct input[type="checkbox"]').attr('checked', false);
	...
	function reorderQuestionAnswers(tableid)
	{
		...
		$j('#'+ tableid + ' .wpcw_quiz_row_answer').each(function(row)
		{
			...
			$j(this).find('.wpcw_quiz_details_tick_correct input[type="checkbox"]').val(count);
	...
	// Show or hide all cert details
	function toggleView_cert_details() {
		if ( ! $j('.wpcw_course_opt_use_certificate').length ) {
			return;
		}
		var certDetails = $j('.wpcw_course_opt_use_certificate:checked').val();
		...
		$j('.wpcw_cert_signature_type').closest('tr').toggle( certDetails == 'use_certs' );
		$j('.wpcw_cert_signature_type_text').closest('tr').toggle( ( certDetails == 'use_certs' && certType == 'text' ) );
		$j('.wpcw_cert_signature_type_image').closest('tr').toggle( ( certDetails == 'use_certs' && certType != 'text' ) );
		$j('.wpcw_cert_logo_enabled').closest('tr').toggle( certDetails == 'use_certs' );
		$j('.wpcw_cert_logo_url').closest('tr').toggle( ( certDetails == 'use_certs' && certLogo == 'cert_logo' ) );
		$j('.wpcw_cert_background_type').closest('tr').toggle( certDetails == 'use_certs' );
		$j('.wpcw_cert_background_custom_url').closest('tr').toggle( ( certDetails == 'use_certs' && bgCustom == 'use_custom' ) );
		$j('.wpcw_course_settings_cert_tools_preview_tr').toggle( certDetails == 'use_certs' );
	}
	...
	// Has the use of certificate been changed?
	$j('.wpcw_course_opt_use_certificate').click(function(){
		toggleView_cert_details();
	});
	...
	$j('#wpcw_form_settings_certificates,#break_course_certificates_user_course').on("click", ".wpcw_insert_image", function(e)
	{
	    e.preventDefault();
	...
	{
		// Don't continue if on the edit question
		if ($j('#wpcw_quiz_details_questions.edit_question_single_details').length) {
			return;
		}
	...
	function toggleView_quiz_useRecommended()
	{
		...
		$j('.wpcw_quiz_details_modify_quiz_recommended_score_tr .subelement_wrapper_all').each(function()
		{
			...
			if ($j(this).closest('.radio_item').find('input[name="quiz_recommended_score"]').is(':checked')) {
				$j(this).slideDown('fast');
				$j('.wpcw_quiz_details_modify_quiz_attempts_allowed_tr').show();
			}  else {
				$j(this).slideUp('fast');
				$j('.wpcw_quiz_details_modify_quiz_attempts_allowed_tr').hide();
			}

			if ($j('.wpcw_quiz_details_modify_quiz_type_tr').find('input[value="quiz_block"]').is(':checked'))
			{
				$j('.wpcw_quiz_details_modify_quiz_attempts_allowed_tr').show();
			}
		});
	...
	// Force check to happen for quiz attempts to show up in context with a recommended score
	$j('#wpcw_tab_wpcw_section_break_quiz_logic').click(function(e)
	{
		toggleView_quiz_useRecommended();
	});
	...
	// Remove option for randomized answers for survey questions
	function toggleView_survey_remove_randomAnswer_option()
	{
		if ($j('.wpcw_quiz_details_modify_quiz_type_tr').find('input[value="survey"]').is(':checked') )
		{
			$j('.wpcw_quiz_details_randomize_answers').closest('tr').hide();
		}
	}
		// Remove random answer option when new question is added
		$j('#wpcw_add_question_multi').click(function(e)
		{
			toggleView_survey_remove_randomAnswer_option();
		});
		// Remove random answer option when question tab is clicked
		$j('#wpcw_tab_wpcw_section_break_quiz_questions').click(function(e)
		{
			toggleView_survey_remove_randomAnswer_option();
		});
	 ...
	 */
	$j('#wpcw_quiz_details_modify input[type="submit"],#wpcw_quiz_details_questions input[type="submit"]').click(function(e)
	{
		...
		//reset error markers for duplicate answers
		$j('.dup_label').remove();
		$j('.wpcw_quiz_row_answer ').css("background", "none");
		...
		$j('.wpcw_dragable_question_holder li, .wpcw_question_holder_static li').each(function(row)
		{
			...
			if (!surveyMode)
			{
				...
				if ($j(this).hasClass('wpcw_question_type_multi'))
				{
					...
					var answerRadioName = $j(this).find('.wpcw_quiz_row_answer input[type="checkbox"]').attr('name');
					var fld_no_answer = false;
					$random_ans_no=$j(this).find('.wpcw_quiz_details_randomize_answers input[type="text"]').val();

					$count=jQuery('input[name="'+answerRadioName+'"]:checkbox:checked').length;
					if($random_ans_no < $count){
						$err_elem=$j(this).find('.wpcw_quiz_details_randomize_answers input[type="text"]').attr('name');
						jQuery('input[name="'+$err_elem+'"]').closest('tr').css("background-color", "#FFEBE8");
						errorCount_questions++;
						$fld_no_answer = true;
					}
					// See if any of the answers are missing selections or not.
					if (!$j('#' + $j(this).attr('id') + ' input:checkbox[name="' + answerRadioName + '"]').is(":checked")) {
						errorCount_questions++;
						fld_no_answer = true;
					}
					...
					// Iterate through all possible answers for this question
					$j(this).find('.wpcw_quiz_row_answer' ).each(function(row){
						var answerName = $j(this).find('input[type="text"]').attr('name');
						var answer = $j('.wpcw_quiz_row_answer input[name="'+ answerName + '"]').val();
						// Get array of answers to check
						answers.push(answer);

						//Get index of value
						var answerIndex = jQuery.inArray(answer,answers);
						//Highlight duplicate answers
						if ( jQuery.inArray(answer,answers,answerIndex+1) !== -1){
							$j('input[name="'+answerName+'"]').closest('.wpcw_quiz_row_answer ').css("background-color", "#FFEBE8");
							$j('input[name="'+answerName+'"]').closest('td').append( '<p class="dup_label" style="background-color:#FFEBE8;color:red;"><strong>Duplicate Answer</strong></p>' );
							errorCount_questions++;
							$fld_no_answer = true;
						}
					});
	...
	// User/Course Enrolment page - show date picker if they're enroled by changing class of whole row.
	$j('#wpcw_tbl_course_access_summary .allowed_access input[type="checkbox"]').click(function(e)
	{
		// If the checkbox is ticked (user has access), then show the date picker by adding wpcw_user_has_access to row class.
		if ($j(this).is(':checked')) {
			$j(this).closest('tr').addClass('wpcw_user_has_access').removeClass('wpcw_user_has_no_access');
		} else {
			$j(this).closest('tr').addClass('wpcw_user_has_no_access').removeClass('wpcw_user_has_access');
		}
	});
