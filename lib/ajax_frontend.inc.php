...
/**
 * Function called when the user is enrolling via enrollment shortcode.
 */
function WPCW_AJAX_course_handleEnrollment_button()
{
	...
	foreach ($courseIDs as $courseID)
	{
		...
		if ($userCourses){
			echo WPCW_UnitFrontend::message_createMessage_success(__('Success! You have been enrolled into', 'wp-courseware') . ' ' . $courseDetails->course_title);
		}else{
			echo WPCW_UnitFrontend::message_createMessage_error(__('Oops! Something went wrong. Please contact the course instructor for more information.', 'wp-courseware'));
		}
	}
	die();
}
