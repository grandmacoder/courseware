...
function WPCW_data_export_gradebookData() {
	...
	foreach ( $userData as $userObj ) {
		...
		$dataToOutput[ 'cumulative_grade' ]    = ( $quizScoresSoFar_count > 0 ? number_format( ( $quizScoresSoFar / $quizScoresSoFar_count ), 1 ) . '%' : __( 'n/a', 'wp-courseware' ) );
		...
		$userProgress                      = new UserProgress( $courseDetails->course_id, $userObj->ID );
