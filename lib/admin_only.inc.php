...
function WPCW_docs_showSupportInfo_News($page)
{
	...
	//$content .= '<li class="googleplus"><a href="https://plus.google.com/115118636946624261241">'.	__( 'Circle Fly Plugins on Google+', 'wp-courseware' ).'</a></li>';
	//$content .= '<li class="rss"><a href="http://feeds.feedburner.com/FlyPlugins">'.	__( 'Subscribe with RSS', 'wp-courseware' ).'</a></li>';
	//$content .= '<li class="email"><a href="http://feedburner.google.com/fb/a/mailverify?uri=FlyPlugins&amp;loc=en_US">'.__( 'Subscribe by email', 'wp-courseware' ).'</a></li>';
	$content .= '</ul>';
...
function WPCW_users_processUserResetAbility_showSuccess()
{
	...
	{
		printf('<div id="message" class="updated"><p>%s</p></div>', __('User progess has been reset.', 'wp-courseware'));
	}
}
...
function WPCW_units_saveUnitPostMetaData($post_id, $post)
{
	...
	$convertedTimestamp = WPCW_arrays_getValue($_POST, 'wpcw_units_drip_interval') + 0;
...
function WPCW_units_manageColumns($column_headers)
{
    ...
    // Add new columns
    $column_headers['wpcw_col_module_and_course'] 	= __('Associated Module &amp; Course', 'wp-courseware');
    $column_headers['wpcw_col_quiz'] 				= __('Associated Quiz', 'wp-courseware');
...
/**
 * Build filter drop down menu for course units
 * Filter will be based on course ID
 */
function WPCW_course_unit_course_filter() {
	global $typenow;

	//Get a list of courses
	$courses = WPCW_courses_getCourseList();

	//Check custom post type
	if( $typenow == 'course_unit' ){

		//Build  Dropdown Menu
 		echo "<select name='course_id' id='course_list' class='postform'>";
		echo "<option value=''>Show All Course Units</option>";

		//Build Menu items for each course
		foreach ($courses as $course_id => $course_title) {
			...
			if($units)
			{
				//Build menu item for each course
				echo '<option value=' . $course_id, isset($_GET["course_id"]) && $_GET["course_id"] == $course_id ? ' selected="selected"' : '' ,'>' . $course_title .' (' . count($units) . ' - units)</option>';
			}
		}

		// Closing tag for drop down menu
		echo "</select>";
	}
}
...
function WPCW_course_unit_course_filter_query($query)
{
	...
	if (isset($_GET['course_id']) && $_GET['course_id'] != '' && $typenow == 'course_unit')
	{
		//Get units associated with course
		$courseMap = new WPCW_CourseMap();
		$courseMap->loadDetails_byCourseID($_GET['course_id']);
		$units = $courseMap->getUnitIDList_forCourse();

		//Filter query
		$var = &$query->query_vars['post__in'];
		if ( isset( $var ) )
		{
			$var = $units;
		}
...
function WPCW_course_unit_permissions_filter_query($query) {
	...
	// Check
	if ( 'edit.php' != $pagenow || 'course_unit' != $typenow ) {
		return;
	}
...
function WPCW_course_unit_permissions_change_counts( $current_screen ) {
	...
	// Check
	if ( $current_screen->id != 'edit-course_unit') {
	...
	// Check permission
	if ( ! user_can( $current_user->ID, 'manage_wpcw_settings' ) ) {
		add_filter( "views_{$current_screen->id}", 'WPCW_course_unit_permissions_list_table_views_filter' );
	}
...
function WPCW_units_showEditScreenMetaBoxes()
{
	...
	add_meta_box(
        ...
        __( 'Convert Post to Course Unit', 'wp-courseware' ),
    ...
    add_meta_box(
        'wpcw_units_convert_post',
        __( 'Convert Page to Course Unit', 'wp-courseware' ),
        'WPCW_units_metabox_showConversionTool',
        'page',
        'side',
        'low'
    ...
    add_meta_box(
        ...
        __( 'Course Unit Template', 'wp-courseware' ),
        'WPCW_units_metabox_showTemplateSelectionTool',
        'course_unit',
        'side',
        'default'
    );

    // Course Units - content drip selection
    add_meta_box(
        'wpcw_units_contentfeed_options',
        __( 'Content Drip', 'wp-courseware' ),
        'WPCW_units_metabox_showContentDripSelectionTool',
        'course_unit',
        'side',
        'default'
    );
...
function WPCW_units_metabox_showConversionTool()
{
	...
	?><p>
	<?php printf(__('Click to <a href="%s">convert this <b>%s</b> to a Course Unit</a>.', 'wp-courseware'), $conversionURL, get_post_type($post->ID)); ?>
...
function WPCW_units_metabox_showContentDripSelectionTool()
{
	...
	$unitMeta = WPCW_units_getUnitMetaData($post->ID);
	...
	if ($unitMeta->unit_drip_date_ts > 1) {
		$convertedDate_Visible = date_i18n('j M Y H:i:s', $unitMeta->unit_drip_date_ts);
		$convertedDate_Hidden  = date_i18n('Y-m-d H:i:s', $unitMeta->unit_drip_date_ts);
	}

	// Not got a specific date, we've not specified it. So set to today.
	else {
		$convertedDate_Visible = date_i18n('j M Y H:i:00');
		$convertedDate_Hidden  = date_i18n('Y-m-d H:i:00');
	}

	// Calculate the interval from a timestamp interval into a human-readable number
	$interval_type  = $unitMeta->unit_drip_interval_type;
	$interval_number = 0;
	switch ($interval_type)
	{
		case 'interval_hours':
			$interval_number = $unitMeta->unit_drip_interval / WPCW_TIME_HR_IN_SECS;
		break;

		case 'interval_days':
			$interval_number = $unitMeta->unit_drip_interval / WPCW_TIME_DAY_IN_SECS;
		break;

		case 'interval_weeks':
			$interval_number = $unitMeta->unit_drip_interval / WPCW_TIME_WEEK_IN_SECS;
		break;

		case 'interval_months':
			$interval_number = $unitMeta->unit_drip_interval / WPCW_TIME_MONTH_IN_SECS;
		break;

		case 'interval_years':
			$interval_number = $unitMeta->unit_drip_interval / WPCW_TIME_YEAR_IN_SECS;
		break;
	}
	...
	printf('<div class="wpcw_units_drip_section">');
		...
		echo WPCW_forms_createDropdown('wpcw_units_drip_type', $delayOptions, $unitMeta->unit_drip_type, 'wpcw_units_drip_type');
	printf('</div>');

	// ### 2 - For a specific date, show a date time picker
	printf('<div class="wpcw_units_drip_section wpcw_datepicker_wrapper" id="wpcw_units_drip_type-drip_specific">');
		...
		printf('<input type="text" id="wpcw_units_drip_date_vis" name="wpcw_units_drip_date_vis" class="wpcw_datepicker_vis" value="%s" />', 	$convertedDate_Visible);
		printf('<input type="hidden" id="wpcw_units_drip_date" name="wpcw_units_drip_date" class="wpcw_datepicker_nonvis" value="%s" />', 		$convertedDate_Hidden);
	printf('</div>');

	// ### 3 - For a relative date, show a digit and a number of days/weeks
	printf('<div class="wpcw_units_drip_section" id="wpcw_units_drip_type-drip_interval">');
		...
		echo WPCW_forms_createDropdown('wpcw_units_drip_interval_type', $intervalOptions, $interval_type, 'wpcw_units_drip_interval_type');
	printf('</div>');
